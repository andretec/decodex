...
if len(sys.argv) < 2:
    print("Uso: python vectorTec.py arquivo.gcode")
    sys.exit(1)

# Arquivo de entrada vindo da linha de comando
file = sys.argv[1]
...
def filtrar_movimentos_zero_antes_de_1(pontos):
    resultado = []
    zeros_pendentes = []
    ...
    for ponto in pontos:
        _, _, shutter, _ = ponto
        if shutter == 0:
            zeros_pendentes.append(ponto)
        else:
            if zeros_pendentes:
                resultado.append(zeros_pendentes[-1])
                zeros_pendentes = []
            resultado.append(ponto)

    if zeros_pendentes:
        resultado.extend(zeros_pendentes)
...
with open(output_file, "w") as f:
    for layer in sorted(dados.keys()):
        ...
        for x, y, s, v in filtrar_movimentos_zero_antes_de_1(dados[layer]):
            f.write(f"{x}\t{y}\t{s}\t{v}\n")
        f.write("\n")
